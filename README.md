...
**7. CHANGE LOG:** Updates to this software.
...
As you probably noticed from my earlier description of these corrections, we are assuming that the distance of a *return bin* is the hypotenuse of a right triangle where one leg is the depth and the other the distance along the bottom. This is only true if the bottom is flat (if you recall, triangles are made of three straight lines). Fancier units (read multi-beam) can correct for this, but you are using a *consumer-grade* fish finder to do research. Upward slopes and things projecting up from the bottom will appear closer than they actually are, whereas downward slopes will appear farther away. Although this assumption is two-dimensional, so if the bottom is flat within a single scan, it will generally be correct. 
